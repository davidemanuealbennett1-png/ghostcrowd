  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (const a of agents) {
        if (a.reached) {
          a.x = 0.5 + Math.random() * 1.0
          a.y = 1 + Math.random() * 13
          a.reached = false
          continue
        }
        ...
        const speed = 0.8
        a.vx += (dx/d * speed - a.vx) * 0.05
        a.vy += (dy/d * speed - a.vy) * 0.05
        ...
        a.x += a.vx * 0.016
        a.y += a.vy * 0.016
        a.x = Math.max(0.2, Math.min(19.8, a.x))
        a.y = Math.max(0.2, Math.min(14.8, a.y))

        const nearby = agents.filter(b => !b.reached && Math.hypot(b.x-a.x,b.y-a.y) < 1.5).length
        const r2 = Math.min(255, nearby * 40)
        const g2 = Math.max(50, 200 - nearby * 30)
        const b2 = Math.max(50, 200 - nearby * 30)
    ...
    <div className="landing">
      ...
      <div className="hero">
        ...
        <p className="hero-sub">
          Sketch any floor plan. Watch hundreds of simulated people flow through it.
          Find the bottlenecks before your customers do.
        </p>
      ...
      <section id="how" className="section">
        ...
        <div className="steps">
          {[
            { n: '01', title: 'Sketch your space', body: 'Draw walls, place tables, mark doors. Use a template or start from scratch. Grid snapping keeps everything clean.' },
            ...
            { n: '03', title: 'Read the results', body: 'See where crowds form. Toggle the heat map. Yellow markers flag exact bottleneck zones. Adjust and rerun.' },
          ].map(s => (
          ...
          {[
            {
              ...
              dimmed: ['Unlimited floor plans', 'Exportable reports', 'Up to 500 agents'],
              cta: 'Get started free', ctaStyle: 'outline', featured: false,
            },
            {
              tier: 'Pro', price: '$19', sub: '/month',
              desc: 'For business owners who need real answers.',
              features: ['Unlimited floor plans', 'Up to 500 agents', 'Heat maps + bottleneck detection', 'Export PNG reports', 'Save & share simulations', 'Priority support'],
              dimmed: ['Team collaboration'],
              cta: 'Start free trial', ctaStyle: 'primary', featured: true,
            },
            {
              tier: 'Business', price: '$49', sub: '/month',
              desc: 'For consultants and multi-location operators.',
              features: ['Up to 2,000 agents', 'Team collaboration', 'Custom branding on reports', 'White-label PDF export', 'API access'],
              dimmed: [],
              cta: 'Contact us', ctaStyle: 'outline', featured: false,
            },
          ].map(p => (
